        ...
        <div class="sidebar-content">
            <ul class="nav nav-secondary">
                ...
                @foreach ($menus as $menu)
                    @php
                        $menuActive = isActiveMenu($menu, $activeMenuName, '', '');
                        // dd($menuActive);
                    @endphp
                    ...
                    @foreach ($menu->submenus as $submenu)
                        @php
                            $submenuActive = isActiveMenu($submenu, '', '', $activeSubSubmenuName);
                        @endphp
                        <li class="nav-item {{ $submenuActive ? 'active' : '' }} submenu">
                            <a data-bs-toggle="collapse" href="#{{ $submenu->id ?? '' }}">
                                <i class="{{ $submenu->icon }}"></i>
                                <p>{{ $submenu->name }}</p>
                                <span class="caret"></span>
                            </a>
                            <div class="collapse {{ $submenuActive ? 'show' : '' }}" id="{{ $submenu->id ?? '' }}">
                                <ul class="nav nav-collapse">
                                    @foreach ($submenu->subsubmenus as $subsubmenu)
                                        @php
                                            $url =
                                        ...
                                        <li
                                            class="{{ isActiveMenu($subsubmenu, '', '', $activeSubSubmenuName) ? 'active' : '' }}">
